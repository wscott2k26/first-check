...
// Synchronize only the exact dependency pins carried by the deterministic store contract.
{
  const rel = 'scripts/store-release-contract.mjs';
  if (fs.existsSync(p(rel))) {
    let source = read(rel);
    source = source
      .split("expo:'~57.0.9'").join("expo:'~57.0.15'")
      .split("'expo-router':'~57.0.9'").join("'expo-router':'~57.0.15'")
      .split("'expo-constants':'~57.0.8'").join("'expo-constants':'~57.0.13'")
      .split("'expo-linking':'~57.0.4'").join("'expo-linking':'~57.0.7'");
    write(rel, source);
  }
}
